...
- 텔레메트리는 전용 SSH 연결에서 동작하며 끊기면 지수 백오프로 자동 재연결
...
2. **접속** — 최초 접속 시 서버의 SHA-256 호스트 키 지문을 보여주고 신뢰 여부를 확인받습니다.
3. **작업** — 터미널에 입력하고, SFTP 패널 사이로 파일을 끌어다 놓고, 하단 바에서 실시간 지표를 확인하세요. CPU / RAM / Disk / GPU / Users를 클릭하면 드래그·크기 조절 가능한 상세 팝오버가 열립니다.
...
- 활성 터미널 세션은 1개 (탭 확장을 위해 세션 ID 구조는 이미 마련됨)
