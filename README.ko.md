...
- **네이티브 로컬 터미널** — SSH 없이 현재 컴퓨터의 셸을 열고 동일한 터미널 분할과 모니터링 UI 사용
...
- 다운로드 파일은 임시 파일에 쓴 뒤 원자적으로 교체 — 불완전한 파일이 노출되지 않음
- 파일 교체/폴더 병합 확인, 삭제, Open 옆 상황별 폴더 생성 버튼, OS 네이티브 폴더 선택기
...
- 저장 세션의 비밀번호 입력란에는 보안 마스크만 표시하고 실제 비밀값은 웹뷰로 전달하지 않음; 빈 상태로 연결하면 보안 저장소 값을 재사용
- 최초 접속 시 SHA-256 호스트 키 지문을 확인하는 TOFU 프롬프트, 지문 불일치 시 연결 차단
- 프로덕션·개발 창 모두에 제한적인 Tauri Content Security Policy 적용

> **1.1.2-beta 업그레이드 안내:** 이제 GpuTerm은 macOS Keychain, Windows Credential Manager, Linux Secret Service/libsecret에 접근하지 않습니다. 기존 OS 보안 저장소 항목은 삭제하지 않지만 자동으로 가져오지도 않으므로, 업그레이드 후 GpuTerm 마스터 비밀번호를 만들고 SSH 비밀번호를 다시 입력하세요.
...
1. **로컬 보안 저장소 잠금 해제** — 최초 실행 시 GpuTerm 마스터 비밀번호를 만들고, 이후 실행부터는 앱을 켤 때 한 번 입력해 저장된 자격증명을 잠금 해제합니다.
2. **프로필 생성** — 사이드바에 host, port, username과 비밀번호 또는 개인키 경로를 입력합니다. **New**로 새 프로필을 시작하고 **Save**로 저장합니다. 경유 접속이 필요하면 저장된 프로필을 **Jump host**로 지정하세요.
...
- 여러 파일이나 폴더를 한 번에 드롭할 수 있으며, 최상위 항목마다 진행률·상태·오류를 개별 보고하는 큐 항목이 됩니다. 폴더 내용은 재귀적으로 전송됩니다.
...
원격 설정은 헬퍼를 셸 명령에 embed하지 않고 SFTP로 전송하며, 모든 쓰기는 임시 파일에 쓴 뒤 unlink-후-rename으로 교체합니다(libssh2는 SFTP 버전 3에 고정되어 있고 v3의 rename은 기존 파일을 덮어쓸 수 없습니다). embed 방식은 신뢰할 수 없었습니다 — macOS/BSD `base64`는 위치 입력 파일을 거부하고, Windows PowerShell 형태는 와이어에서 약 23,000자에 달해 cmd.exe의 8,191자 한계를 넘습니다(Windows OpenSSH는 관리자가 `DefaultShell`을 바꾸지 않으면 exec 요청에 cmd.exe를 씁니다). 헬퍼와 `settings.json` 모두 임시 파일에 쓴 뒤 이름을 바꿔 넣으므로, 설정이 실패해도 동작하던 헬퍼가 빈 파일로 남지 않습니다. Windows 편집기와 PowerShell 리다이렉션이 붙이는 UTF-8 BOM도 허용합니다.
...
src-tauri/src/ssh/      Rust 백엔드
  ...
  agent_monitor.rs      에이전트 프로세스 트리 + 읽기 전용 세션 메타데이터
...
비밀번호와 키 패스프레이즈는 `credentials.enc`에 저장됩니다. GpuTerm은 마스터 비밀번호를 Argon2id(64 MiB, 3회 반복)로 256비트 키로 유도하고, 쓸 때마다 새 무작위 nonce를 사용해 전체 페이로드를 AES-256-GCM으로 암호화·인증합니다. 마스터 비밀번호와 키는 현재 실행 중에만 메모리에 유지하며, `sessions.json`에는 host 등의 메타데이터와 개인키의 *경로*만 들어갑니다. 프로필을 삭제하면 해당 보안 항목도 삭제됩니다.

이전 Keychain/Credential Manager/Secret Service 항목은 가져오거나 접근하지 않습니다. 업그레이드 후 SSH 비밀번호를 다시 입력하세요. 마스터 비밀번호를 잊으면 보안 저장소를 초기화할 수 있으며, 프로필은 유지되지만 저장 비밀번호는 다시 입력해야 합니다. [데이터 저장 위치](#아키텍처) 표를 참고하세요.
