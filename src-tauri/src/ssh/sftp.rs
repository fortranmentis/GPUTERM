use crate::ssh::session::{open_ssh_session, target_for_active_session, AppState, SshTarget};
...
pub fn sftp_list_dir(
    ...
    let real_path = sftp.realpath(remote_path).unwrap_or_else(|_| PathBuf::from(requested));
    let mut entries = sftp
        .readdir(remote_path)
        .map_err(|error| format!("SFTP directory listing failed for {}: {}", requested, error))?
        .into_iter()
...
pub async fn sftp_download_file(
    ...
    tauri::async_runtime::spawn_blocking(move || download_file(app, target, request))
        .await
...
pub async fn sftp_upload_file(
    ...
    let target = target_for_active_session(&state, &request.session_id)?;
    tauri::async_runtime::spawn_blocking(move || upload_file(app, target, request))
        .await
        .map_err(|error| format!("SFTP upload task failed: {}", error))?
}

#[tauri::command]
pub fn sftp_delete(state: State<AppState>, request: SftpPathRequest) -> Result<(), String> {
    let target = target_for_active_session(&state, &request.session_id)?;
    let session = open_ssh_session(&target)?;
    let sftp = session
        .sftp()
...
#[tauri::command]
pub fn sftp_mkdir(state: State<AppState>, request: SftpPathRequest) -> Result<(), String> {
    let target = target_for_active_session(&state, &request.session_id)?;
    let session = open_ssh_session(&target)?;
    let sftp = session
        .sftp()
...
fn download_file(
    ...
    let mut local_file = fs::File::create(local_path)
        .map_err(|error| format!("Failed to create local file {}: {}", request.local_path, error))?;
