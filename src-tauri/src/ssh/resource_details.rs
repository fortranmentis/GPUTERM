use crate::ssh::session::{open_ssh_session, target_for_active_session, AppState};
...
pub async fn get_resource_details(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        session.set_timeout(DETAIL_TIMEOUT_MS);

        match collect_details(&session, resource_type) {
...
fn parse_gpu_detail_output(
    ...
    for gpu in &mut metrics {
        gpu.processes.sort_by(|a, b| b.used_memory_mi_b.cmp(&a.used_memory_mi_b));
    }
    Ok(metrics)
}
...
fn split_sections(output: &str) -> HashMap<String, String> {
    ...
    for line in output.lines() {
        if let Some(section) = line.trim().strip_prefix("__").and_then(|value| value.strip_suffix("__")) {
...
fn required_section<'a>(sections: &'a HashMap<String, String>, name: &str) -> Result<&'a str, String> {
    sections
        .get(name)
        .map(String::as_str)
        .filter(|value| !value.trim().is_empty())
        .ok_or_else(|| format!("missing {} detail section", name))
}

fn parse_meminfo_values(output: &str) -> HashMap<String, u64> {
    output
        .lines()
        .filter_map(|line| {
...
fn parse_loadavg(output: &str) -> (Option<f64>, Option<f64>, Option<f64>) {
...
fn parse_average_clock(cpuinfo: &str) -> Option<f64> {
    let values = cpuinfo
        .lines()
        .filter_map(|line| {
            let (key, value) = line.split_once(':')?;
...
fn parse_lscpu_value(output: &str, key: &str) -> Option<String> {
