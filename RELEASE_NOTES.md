...
- Fixed the native Windows Claude status-line helper writing its snapshot and then returning an empty line. The helper now uses the `OrderedDictionary` membership API supported by both Windows PowerShell 5.1 and PowerShell 7.
