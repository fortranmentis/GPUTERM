...
- **AGY, Codex, and Claude Code monitoring** — aggregate CPU/RAM and elapsed time across each CLI's complete child-process tree, then show provider-specific session, model, tokens, used/remaining context, work, cost, and available rate-limit metadata
...
- **Agents:** the System modes include a separate AGY/Codex/Claude Code card. Its CPU and memory values include descendants such as language servers, subagents, and background commands rather than only the launcher process. AGY 1.0 conversation metadata and Claude Code status-line snapshots also supply context remaining; Claude subscriber snapshots can show the remaining 5-hour and 7-day limits.
